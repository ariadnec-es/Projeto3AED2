...
\<p align = "justify"\>Com o grafo limpo, foram calculadas métricas estatísticas para identificar a relevância de cada página na rede. [cite_start]Os resultados foram incorporados como atributos dos nós para posterior visualização. </p\>
...
<p align = "justify"\>A análise final resultou em um grafo limpo contendo aproximadamente **8.858 nós** e **10.596 arestas**. As métricas calculadas revelaram a estrutura de "Mundo Pequeno" da Wikipedia, permitindo identificar hubs globais (como páginas de grandes guerras ou tecnologia) e hubs locais (tópicos específicos de arte). O arquivo GraphML gerado contém todos esses atributos, permitindo filtros avançados no Gephi.\</p\>

### 5.2. Visualização (Gephi)

<p align = "justify"\>Para a representação visual, recomenda-se o uso do algoritmo de layout \<i\>Force Atlas 2\</i\> ou \<i\>Fruchterman Reingold\</i\> no Gephi. As visualizações destacam a estrutura de comunidades e a separação entre o núcleo (K-Core) e a periferia (K-Shell) da rede.\</p\>
